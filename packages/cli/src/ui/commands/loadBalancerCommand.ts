...
async function handleExecuteRequest(
  ...
  if (args.length < 1) {
    return {
      ...
      messageType: 'error',
      content:
        '❌ 使用方法: `/loadbalancer execute <prompt>`\n\n例: `/loadbalancer execute "こんにちは、世界"`',
    };
  }
  ...
  try {
    ...
    if (config.endpoints.length === 0) {
      return {
        ...
        messageType: 'error',
        content:
          '❌ 利用可能なエンドポイントがありません。まずエンドポイントを追加してください。',
      };
    }
 ...
 * アルゴリズムを更新
 */
async function handleUpdateAlgorithm(
  args: string[],
): Promise<MessageActionReturn> {
  try {
    if (args.length < 1) {
...
export const loadBalancerCommand: SlashCommand = {
  ...
  altNames: ['lb'],
  description:
    'ロードバランサー機能 - 複数のGemini APIエンドポイント間でリクエストを分散',
