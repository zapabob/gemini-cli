...
export class GeminiClient {
   ...
   * サブエージェント用のプロンプトを生成
   */
  private generateSubagentPrompt(
    subagent: Subagent,
    task: string,
    context?: string,
  ): string {
    const specialtyDescription = this.getSpecialtyDescription(
      subagent.specialty,
    );
...
タスク: ${task}`;
  ...
  private getSpecialtyDescription(specialty: string): string {
    const descriptions: Record<string, string> = {
      code_review:
        'コードレビューと品質保証の専門家。バグの特定、セキュリティ問題の検出、コード品質の向上に特化',
      debugging:
        'デバッグとトラブルシューティングの専門家。エラーの原因特定と解決策の提案に特化',
      data_analysis:
        'データ分析と統計の専門家。データの可視化、統計解析、インサイトの抽出に特化',
      documentation:
        'ドキュメント作成の専門家。技術文書、API仕様書、ユーザーガイドの作成に特化',
      testing:
        'テスト戦略と品質保証の専門家。ユニットテスト、統合テスト、自動化テストの設計に特化',
      optimization:
        'パフォーマンス最適化の専門家。コード最適化、アルゴリズム改善、リソース効率化に特化',
      security:
        'セキュリティの専門家。脆弱性の特定、セキュリティベストプラクティスの提案に特化',
      architecture:
        'システム設計の専門家。スケーラブルで保守性の高いアーキテクチャの設計に特化',
    };
  ...
  async generateText(request: GeminiRequest): Promise<GeminiResponse> {
    ...
    try {
      ...
      throw new Error(
        `Gemini API request failed: ${_error instanceof Error ? _error.message : String(_error)}`,
      );
    }
  }
  ...
  async executeParallelTasks(
    ...
    const promises = subagents.map(async (subagent) => {
      try {
        const response = await this.executeSubagentTask(
          subagent,
          task,
          context,
          options,
        ...
        throw new Error(
          `Subagent ${subagent.name} failed: ${_error instanceof Error ? _error.message : String(_error)}`,
        );
      }
    });
