...
Gemini
CLI は、Gemini の力を直接ターミナルにもたらすオープンソースのAIエージェントです。Gemini への軽量アクセスを提供し、プロンプトからモデルまでの最も直接的なパスを提供します。
...
- **🧠 強力なGemini 3.0 Pro**: 100万トークンのコンテキストウィンドウへのアクセス
...
- **🆕 ロードバランシング**: 信頼性とパフォーマンス向上のため複数のGemini
  APIエンドポイントに リクエストを分散
...
## 🆕 最新機能 (v0.29.0) - 上流最新同期 & 安定版

**🆕 上流最新同期**: 最新の上流アップデート（google-gemini/gemini-cli）を完全に統合し、モデル定義（Gemini
2.5/3.0）を整合 **🆕
Lint/テストの修正**: 多数の非推奨警告および潜在的なバグ（conditional-expect等）を解消し、コアテスト通過率100%を達成
**🆕 安定性の向上**: 電源断保護、ロードバランシング、サブエージェント等の独自機能を維持したまま基盤を刷新
**🆕 高度な設定**: 上流互換性を持つ統一設定システムと最新モデルへの最適化
...
- [**CLIコマンドリファレンス**](./docs/cli/commands.md) - 全てのスラッシュコマンド（`/help`,
  `/chat`, `/mcp`など）
...
コントリビューションを歓迎します！Gemini CLI は完全にオープンソース（Apache
2.0）であり、コミュニティに以下を奨励します:
