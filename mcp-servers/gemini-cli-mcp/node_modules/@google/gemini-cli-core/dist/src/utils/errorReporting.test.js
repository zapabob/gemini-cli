...
describe('reportError', () => {
    ...
    it('should generate a report and log the path', async () => {
        ...
        const type = 'general';
        const expectedReportPath = path.join(testDir, `gemini-client-error-${type}-${MOCK_TIMESTAMP.replace(/[:.]/g, '-')}.json`);
        await reportError(error, baseMessage, context, type);
        // Verify the file was written
        expect(vi.mocked(fs.writeFile)).toHaveBeenCalledWith(expectedReportPath, expect.stringContaining('"message": "Test error"'));
    ...
    it('should handle stringification failure of report content (e.g. BigInt in context)', async () => {
        ...
        const expectedMinimalReportPath = path.join(testDir, `gemini-client-error-${type}-${MOCK_TIMESTAMP.replace(/[:.]/g, '-')}.json`);
        await reportError(error, baseMessage, context, type);
        expect(debugLoggerErrorSpy).toHaveBeenCalledWith(`${baseMessage} Could not stringify report content (likely due to context):`, expect.any(Error));
        expect(debugLoggerErrorSpy).toHaveBeenCalledWith('Original error that triggered report generation:', error);
