...
export class DeepResearchTool extends BaseDeclarativeTool {
    config;
    constructor(config) {
        super('deep_research', 'Deep Research', 'Perform comprehensive research with multi-level analysis, source validation, and topic exploration', Kind.Search, {
            type: 'object',
            properties: {
                query: {
                    type: 'string',
            ...
            required: ['query'],
        }, config.getMessageBus(), true, // isOutputMarkdown
        true);
        this.config = config;
    }
...
class DeepResearchToolInvocation extends BaseToolInvocation {
    ...
    config;
    messageBus;
    constructor(params, config, messageBus) {
    ...
    async execute(signal, _updateOutput) {
        ...
        const { query, max_depth = 3, max_sources = 10, strategy = 'comprehensive', include_academic = true, recent_years = 5, focus_domains = [], exclude_types = [], } = this.params;
        try {
            // Record telemetry
            ...
            const researchResults = await this.performMultiLevelResearch(researchPrompt, geminiClient, signal, {
                max_depth,
                max_sources,
                strategy,
            });
     ...
     */
    async performMultiLevelResearch(prompt, geminiClient, signal, options) {
        ...
        for (let depth = 1; depth <= max_depth; depth++) {
            ...
            try {
                const response = await geminiClient.generateContent({ model: 'gemini-3.0-flash-exp' }, [{ role: 'user', parts: [{ text: levelPrompt }] }], signal);
    ...
    async saveResearchToMarkdown(query, results, options) {
        try {
            ...
            debugLogger.warn('Failed to save DeepResearch results to markdown:', error);
            return '';
        }
    }
    /**
