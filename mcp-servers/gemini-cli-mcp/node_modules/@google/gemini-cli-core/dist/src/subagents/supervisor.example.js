...
- テスト: Jest, React Testing Library
  `;
    ...
    try {
        // 5. 監督者エージェントの実行
        const result = await supervisor.superviseImplementation(implementationGoal, subagents, context);
        // 6. 結果の表示
        console.log('\n🎉 監督者エージェントの実行が完了しました！\n');
        console.log('📊 実行結果:');
...
- パスワードリセット機能
  `;
    const result = await supervisor.superviseImplementation(authGoal, authSubagents, 'Node.js + Express + PostgreSQL 環境');
...
//# sourceMappingURL=supervisor.example.js.map