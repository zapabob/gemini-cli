...
export class SubagentRegistry {
    ...
    getSubagentsBySpecialty(specialty) {
...
export class SubagentExecutor {
     ...
     * 進捗メッセージを送信
     */
    sendProgress(message, type = 'info', agentId, agentName) {
        if (this.onProgress) {
            // 色分けされたメッセージを生成
    ...
    async executeTask(subagent, task) {
        ...
        try {
            ...
            const agentSpeech = this.colorManager.formatAgentSpeech(subagent.id, subagent.name, subagent.specialty, response.text);
    ...
    async executeParallel(subagents, task) {
        ...
        for (let i = 0; i < chunks.length; i++) {
            ...
            try {
                const chunkResults = await Promise.allSettled(chunk.map((subagent) => executeWithLimit(subagent)));
                // 結果を処理
     ...
     * タスクプロンプト生成
     */
    generateTaskPrompt(subagent, task) {
...
専門的な視点から回答してください。`;
     ...
     * 実行状況の詳細表示
     */
    displayExecutionStatus(subagents, activeCount, completedCount) {
