...
export class MainAgentInterface {
     ...
     * 自律的にサブエージェントの使用を決定
     */
    async executeTask(task, context, mode = 'auto', options) {
     ...
     * 実行モードの自動決定
     */
    async determineExecutionMode(task, context) {
        const prompt = `
...
`;
     ...
     * 自律モードでの実行
     */
    async executeAutonomousMode(task, _context, _options) {
     ...
     * 監督者モードでの実行
     */
    async executeSupervisorMode(task, _context, _options) {
     ...
     * 手動モードでの実行
     */
    async executeManualMode(task, _context, _options) {
     ...
     * 自然言語プロンプト実行モード
     */
    async executeNaturalLanguageMode(task, context, options) {
     ...
     * リアルタイム協調セッションの開始
     */
    async startRealTimeCollaboration(task, _context, _options) {
     ...
     * リアルタイム協調セッションへの参加
     */
    async joinCollaborationSession(sessionId, subagentId) {
    ...
    getPerformanceStats() {
        ...
        // モード別の使用回数をカウント（簡易版）
        const mostUsedMode = 'autonomous'; // 実際の実装では統計を取る
        return {
            totalTasks,
            successfulTasks,
...
//# sourceMappingURL=mainAgentInterface.js.map