...
export class CollaborativeAgentSystem {
     ...
     */
    async executeCollaborativeTask(task, context, _options = {}) {
        ...
        try {
            ...
            const finalResult = await this.mainAgent.integrateResults(task, subagentResults, taskAnalysis);
     ...
     */
    async executeRealTimeCollaboration(task, context, options = {}) {
        ...
        try {
            ...
            // 2. 協調セッション開始
            const session = new CollaborationSession(sessionId, this.mainAgent, this.subagents, this.executor, options);
            // 3. リアルタイム協調実行
...
`;
     ...
     * サブエージェント選択
     */
    async selectSubagents(analysis) {
     ...
     * 最適なサブエージェント選択
     */
    selectBestSubagent(subagents, analysis) {
        // スコアリングシステム
     ...
     * 最近性スコア計算
     */
    calculateRecencyScore(subagent) {
     ...
     * タスク分割
     */
    async divideTask(analysis, subagents) {
        const subtasks = [];
     ...
     * サブタスク作成
     */
    async createSubtask(analysis, subagent) {
        const prompt = `
...
`;
     ...
     * サブエージェント実行
     */
    async executeSubagents(subtasks) {
        const results = [];
...
`;
     ...
     * 結果統合・検証
     */
    async integrateResults(originalTask, subagentResults, analysis) {
        const prompt = `
...
class CollaborationSession {
    ...
    constructor(sessionId, mainAgent, subagents, executor, options) {
     ...
     * リアルタイム協調実行
     */
    async execute(task, initialAnalysis) {
     ...
     * 現在の状況分析
     */
    async analyzeCurrentSituation(task, analysis) {
        const prompt = `
...
`;
     ...
     * 次のアクション決定
     */
    async decideNextAction(situation) {
        // 状況に基づいてアクションを決定
     ...
     * サブエージェントアクション実行
     */
    async executeSubagentAction(action) {
     ...
     * 統合アクション実行
     */
    async executeIntegrationAction(_action) {
        const integratedResult = await this.mainAgent.integrateResults('Current task', [], // 実際の結果を使用
        {});
        return {
            success: true,
     ...
     * 分析アクション実行
     */
    async executeAnalysisAction(action) {
     ...
     * 終了条件チェック
     */
    shouldTerminate(result, analysis) {
        return result.success && analysis.complexity <= 2;
    }
    /**
     * タスク更新
     */
    updateTask(currentTask, result) {
        return `${currentTask}\n\n最新の結果: ${result.data}`;
    }
    /**
     * 最終結果生成
...
//# sourceMappingURL=collaborativeAgent.js.map