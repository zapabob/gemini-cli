...
export class CheckpointManager {
     ...
     * チェックポイントディレクトリの初期化
     */
    async initializeCheckpointDirectory() {
        try {
     ...
     * チェックポイント保存
     */
    async saveCheckpoint(sessionId, data, isFinal = false) {
        try {
     ...
     * ファイルへの保存
     */
    async saveToFile(checkpointData) {
    ...
    async emergencySave() {
        ...
        try {
            // 全アクティブセッションの緊急保存
            const savePromises = Array.from(this.activeSessions.entries()).map(async ([sessionId, session]) => {
                try {
     ...
     * 復旧時間推定
     */
    estimateRecoveryTime(checkpointCount) {
