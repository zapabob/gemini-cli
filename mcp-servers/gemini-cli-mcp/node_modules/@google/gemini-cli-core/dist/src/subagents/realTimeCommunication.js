...
export class RealTimeCommunicationSystem extends EventEmitter {
     ...
     * 受信メッセージの処理
     */
    handleIncomingMessage(_agentId, _data) {
        try {
     ...
     * タスク割り当てメッセージの送信
     */
    async assignTask(taskId, task, subagentId, context, priority = 'medium') {
        const message = {
            id: this.generateMessageId(),
     ...
     * タスク進捗メッセージの送信
     */
    async reportProgress(taskId, progress, status, agentId, currentStep) {
        const message = {
            id: this.generateMessageId(),
            type: 'task_progress',
            timestamp: new Date().toISOString(),
     ...
     * タスク完了メッセージの送信
     */
    async reportCompletion(taskId, result, executionTime, tokensUsed, qualityScore, confidenceLevel, agentId) {
        const message = {
            id: this.generateMessageId(),
     ...
     * 協調要求メッセージの送信
     */
    async requestCoordination(requestType, taskId, description, urgency, senderId, targetId) {
        const message = {
            id: this.generateMessageId(),
     ...
     * メインエージェント指示メッセージの送信
     */
    async sendDirective(directiveType, instruction, targetSubagentId, taskId, priority = 'medium') {
        const message = {
            id: this.generateMessageId(),
     ...
     * パフォーマンスメトリクスの送信
     */
    async sendPerformanceMetrics(agentId, cpuUsage, memoryUsage, responseTime, throughput, errorRate, activeTasks) {
        const message = {
            id: this.generateMessageId(),
     ...
     * セッション制御メッセージの送信
     */
    async controlSession(action, reason) {
        const message = {
            id: this.generateMessageId(),
     ...
     * キューされたメッセージの送信
     */
    async sendQueuedMessages(agentId) {
        const queue = this.messageQueue.get(agentId);
