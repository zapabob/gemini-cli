...
async function loadYamlModule() {
    ...
    try {
        ...
        throw new Error(`YAML support is required to load subagents. Install the "yaml" package to continue. (Original error: ${error instanceof Error ? error.message : String(error)})`);
    }
}
/**
...
export class YamlAgentLoader {
    agentsDir;
    registry;
    constructor(agentsDir = './.gemini/agents') {
    ...
    async loadAllAgents() {
        if (!existsSync(this.agentsDir)) {
            console.log(`サブエージェントディレクトリが存在しません: ${this.agentsDir}`);
            return;
        }
        ...
        for (const file of yamlFiles) {
            try {
                ...
                console.error(`サブエージェント定義の読み込みに失敗しました: ${file}`, error);
            }
        }
    }
    /**
    ...
    async loadAgentDefinition(filename) {
        ...
        try {
            ...
            const name = typeof yamlContent.name === 'string' ? yamlContent.name : undefined;
            const description = typeof yamlContent.description === 'string'
                ? yamlContent.description
                : undefined;
            const specialty = typeof yamlContent.specialty === 'string'
                ? yamlContent.specialty
                : undefined;
            const model = typeof yamlContent.model === 'string'
                ? yamlContent.model
                : 'gemini-3.0-pro';
            const color = typeof yamlContent.color === 'string' ? yamlContent.color : 'blue';
            ...
            const capabilities = Array.isArray(yamlContent.capabilities)
                ...
                : [];
            const config = yamlContent.config && typeof yamlContent.config === 'object'
                ? yamlContent.config
                : {};
            // YAMLの検証と変換
            if (!name || !description || !specialty) {
                throw new Error('必須フィールドが不足しています: name, description, specialty');
            }
            const definition = {
                name,
                description,
     ...
     * 新しいサブエージェント定義を作成する
     */
    async createAgentDefinition(name, specialty, description) {
        const yaml = await loadYamlModule();
        const definition = {
            name,
            description,
     ...
     * サブエージェント定義を削除する
     */
    async deleteAgentDefinition(name) {
        const filename = `${name}.yaml`;
        const filePath = join(this.agentsDir, filename);
        try {
...
//# sourceMappingURL=yamlAgentLoader.js.map