...
export class SubAgentScope {
    ...
    static async create(name, runtimeContext, promptConfig, modelConfig, runConfig, options = {}) {
        const subagentToolRegistry = new ToolRegistry(runtimeContext, runtimeContext.getMessageBus());
        if (options.toolConfig) {
            ...
            for (const tool of subagentToolRegistry.getAllTools()) {
                ...
                if (requiredParams.length > 0) {
                    ...
                    debugLogger.warn(`Cannot check tool "${tool.name}" for interactivity because it requires parameters. Assuming it is safe for non-interactive use.`);
                    continue;
                }
    ...
    async runNonInteractive(context) {
        ...
        try {
            ...
            while (true) {
                ...
                const responseStream = await chat.sendMessageStream({ model: this.modelConfig.model }, currentMessages[0]?.parts || [], promptId, abortController.signal);
                const functionCalls = [];
                let textResponse = '';
...
`);
            ...
            if (toolResponse.error) {
                debugLogger.error(`Error executing tool ${functionCall.name}: ${toolResponse.resultDisplay || toolResponse.error.message}`);
            }
    ...
    async createChatObject(context) {
        ...
        try {
            ...
            if (this.toolConfig) {
                ...
                tools.push(...this.toolRegistry.getFunctionDeclarationsFiltered(toolsToLoad));
            }
            if (this.outputConfig && this.outputConfig.outputs) {
                tools.push(...this.getScopeLocalFuncDefs());
            }
            this.runtimeContext.setModel(this.modelConfig.model);
            return new GeminiChat(this.runtimeContext, systemInstruction, [{ functionDeclarations: tools }], start_history);
        }
        catch (error) {
