...
<head>
    <title>Code coverage report for src/utils/googleQuotaErrors.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../index.html">All files</a> / <a href="index.html">src/utils</a> googleQuotaErrors.ts</h1>
        <div class='clearfix'>
            
            <div class='fl pad1y space-right2'>
                <span class="strong">93.84% </span>
                <span class="quiet">Statements</span>
                <span class='fraction'>259/276</span>
            </div>
...
<a name='L374'></a><a href='#L374'>374</a></td><td class="line-coverage quiet"><span class="cline-any cline-yes">1x</span>
...
function parseDurationInSeconds(duration: string): number | null {
  if (duration.endsWith('ms')) {
    ...
    return isNaN(milliseconds<span class="branch-0 cbranch-no" title="branch not covered" >) ? null : m</span>illiseconds / 1000;
  }
  if (duration.endsWith('s')) {
    const seconds = parseFloat(duration.slice(0, -1));
    return isNaN(seconds<span class="branch-0 cbranch-no" title="branch not covered" >) ? null : s</span>econds;
...
&nbsp;
  if (!errorInfo) <span class="branch-0 cbranch-no" title="branch not covered" >{</span>
<span class="cstat-no" title="statement not covered" >    return null;</span>
<span class="cstat-no" title="statement not covered" >  }</span>
&nbsp;
  if (
...
&nbsp;
  if (status === 404) {
    const message =
      googleApiError?.<span class="branch-0 cbranch-no" title="branch not covered" >message ||</span>
<span class="cstat-no" title="statement not covered" >      (error instanceof Error ? error.message : 'Model not found');</span>
...
<span class="branch-0 cbranch-no" title="branch not covered" >        googleApiError ?? {</span>
<span class="cstat-no" title="statement not covered" >          code: 429,</span>
...
&nbsp;
  ...
  if (quotaFailure) {
    for (const violation of quotaFailure.violations) {
      const quotaId = violation.<span class="branch-0 cbranch-no" title="branch not covered" >quotaId ?? '';</span>
      ...
      if (validDomains.includes(errorInfo.domain)) {
        if (errorInfo.reason === 'RATE_LIMIT_EXCEEDED') {
          return new RetryableQuotaError(
            `${googleApiError.message}`,
            googleApiError,
<span class="branch-0 cbranch-no" title="branch not covered" >            delaySeconds ?? 10,</span>
          );
        }
...
&nbsp;
  if (errorInfo) {
    const quotaLimit = errorInfo.metadata?.['quota_limit'<span class="branch-0 cbranch-no" title="branch not covered" >] ?? '';</span>
...
<span class="branch-0 cbranch-no" title="branch not covered" >  }</span>
<span class="cstat-no" title="statement not covered" >  return error; // Fallback to original error if no specific classification fits.</span>
<span class="cstat-no" title="statement not covered" >}</span>
&nbsp;</pre></td></tr></table></pre>
