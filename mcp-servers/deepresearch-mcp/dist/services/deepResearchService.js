...
export class DeepResearchService {
    ...
    async execute(params) {
        try {
            ...
            // 多層研究の実行
            const results = await this.performMultiLevelResearch(researchPrompt, maxDepth, maxSources, strategy);
            // 結果の保存
            const savedFilePath = await this.saveResearchToMarkdown(params.query, results, {
                strategy,
     ...
     * 研究プロンプトの作成
     */
    createResearchPrompt(query, options) {
...
詳細で包括的な研究レポートを作成してください。
     ...
     * 多層研究の実行
     */
    async performMultiLevelResearch(prompt, maxDepth, maxSources, strategy) {
...
この深度での調査を続行してください。
     ...
     */
    async saveResearchToMarkdown(query, results, options) {
     ...
     * Markdownコンテンツの生成
     */
    generateMarkdownContent(query, results, options) {
...
*このレポートは DeepresearchMCPサーバーによって自動生成されました。*
     ...
     * 研究結果のフォーマット
     */
    formatResearchResults(results, options) {
        const display = `
...
//# sourceMappingURL=deepResearchService.js.map